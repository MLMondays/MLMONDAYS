...
sample_data_path= "/media/marda/TWOTB/USGS/SOFTWARE/DL-CDI2020/1_ImageRecog/data/tamucc/subset_2class/sample"
...
#
# ###==============================================================================
# ## Model 1: Transfer Learning model, "vanilla" implementation
#
# input_shape = (TARGET_SIZE, TARGET_SIZE)
#
# #model1 = transfer_learning_mobilenet_model(len(CLASSES), input_shape, dropout_rate=0.5) #greyscale
# model1 = transfer_learning_mobilenet_model(len(CLASSES), input_shape+(3,), dropout_rate=0.5)
#
# # if len(CLASSES)>2:
...
#
# # set checkpoint file
# filepath = 'tamucc_subset_2class_mn2_best_weights_model1.h5'
# model_checkpoint = ModelCheckpoint(filepath, monitor='val_loss',
#                                 verbose=0, save_best_only=True, mode='min',
#                                 save_weights_only = True)
#
...
#
# # without augmentation, call like this
# history1 = model1.fit(get_training_dataset(), steps_per_epoch=steps_per_epoch, epochs=MAX_EPOCHS,
#                       validation_data=get_validation_dataset(), validation_steps=validation_steps,
#                       callbacks=callbacks)
#
# history1 = model1.fit(augmented_train_ds, steps_per_epoch=steps_per_epoch, epochs=MAX_EPOCHS,
#                       validation_data=augmented_val_ds, validation_steps=validation_steps,
#                       callbacks=callbacks)
#
# # Plot training history
...
# plt.subplot(121)
# plt.plot(np.arange(1,n1+1), history1.history['accuracy'], 'b', label='train accuracy')
# plt.plot(np.arange(1,n1+1), history1.history['val_accuracy'], 'k', label='validation accuracy')
# plt.xlabel('Epoch number', fontsize=10); plt.ylabel('Accuracy', fontsize=10)
# plt.legend(fontsize=10)
#
# plt.subplot(122)
# plt.plot(np.arange(1,n1+1), history1.history['loss'], 'b', label='train loss')
# plt.plot(np.arange(1,n1+1), history1.history['val_loss'], 'k', label='validation loss')
# plt.xlabel('Epoch number', fontsize=10); plt.ylabel('Loss', fontsize=10)
# plt.legend(fontsize=10)
#
# # plt.show()
# plt.savefig('tamucc_sample_2class_mv2_model1.png', dpi=200, bbox_inches='tight')
#
# plt.close('all')
# K.clear_session()
#
...
# ##########################################################
# ### evaluate
# loss, accuracy = model1.evaluate(get_validation_eval_dataset(), batch_size=BATCH_SIZE)
# print('Test Mean Accuracy: ', round((accuracy)*100, 2),' %')
#
#
# ##########################################################
...
#     plt.axis('off')
#
#     scores = model1.predict(tf.expand_dims(im, 0) , batch_size=1)
#     n = np.argmax(scores[0])
#     est_name = CLASSES[n].decode()
#     if name==est_name:
#        plt.text(10,50,'prediction: %s' % est_name,
...
#
# # plt.show()
# plt.savefig('tamucc_sample_2class_mv2_model1_est16samples.png',
#             dpi=200, bbox_inches='tight')
# plt.close('all')
#
#
# ## write model out to json (see owg)
#
# ## confusion matrix
